...
function calculatePointsForPrediction(predictionData, resultsById, groupResultsById = {}) {
  let points = 0;
...
if (predictedHome === realHome && predictedAway === realAway) {
  points += 5;
    ...
    const officialGroupRanking = groupResultsById?.[group]?.ranking;

    if (officialGroupRanking || groupHasAllResults(group, resultsById)) {
      const actualRanking = officialGroupRanking || calculateStandings(group, resultsById).map((row) => row.team);
...
function App() {
  ...
  const [adminGroupRanking, setAdminGroupRanking] = useState(groups.A);
...
const organizerTotal = paidPlayers * ORGANIZER_AMOUNT_PER_PLAYER;
  ...
  useEffect(() => {
    setAdminGroupRanking(groupResults[adminGroup]?.ranking || groups[adminGroup]);
  }, [adminGroup, groupResults]);
...
const unsubscribeLeaderboard = onSnapshot(collection(db, 'users'), (snapshot) => {
  ...
  setLeaderboard(rows);
    ...
    const unsubscribeGroupResults = onSnapshot(collection(db, 'groupResults'), (snapshot) => {
      const results = {};
      snapshot.docs.forEach((item) => {
        results[item.id] = item.data();
      });
  ...
  async function recalculateAllPoints(resultsOverride = officialResults, groupResultsOverride = groupResults) {
  if (!isAdmin) {
  ...
  try {
    ...
    await Promise.all(
      predictionsSnapshot.docs.map(async (predictionDoc) => {
        ...
        const points = calculatePointsForPrediction(data.predictions, resultsOverride, groupResultsOverride);
...
function moveAdminGroupTeam(index, direction) {
  const target = index + direction;

  if (target < 0 || target >= adminGroupRanking.length) return;

  const ranking = [...adminGroupRanking];
  [ranking[index], ranking[target]] = [ranking[target], ranking[index]];
  setAdminGroupRanking(ranking);
}

async function saveGroupResult() {
  if (!isAdmin) {
    alert('Only admin can save group results.');
    return;
  }

  if (!adminGroupRanking || adminGroupRanking.length !== 4) {
    alert('Group ranking must have 4 teams.');
    return;
  }
  ...
  const updatedGroupResults = {
    ...
    [adminGroup]: {
      group: adminGroup,
      ranking: adminGroupRanking
    }
  };

  try {
    await setDoc(
      doc(db, 'groupResults', adminGroup),
      {
        group: adminGroup,
        ranking: adminGroupRanking,
        updatedAt: serverTimestamp()
    ...
    await recalculateAllPoints(officialResults, updatedGroupResults);

    alert(`Group ${adminGroup} result saved and leaderboard updated.`);
  } catch (error) {
    console.error('Save group result error:', error);
    alert(`Group result save failed: ${error.message}`);
  }
}

async function deleteGroupResult() {
  if (!isAdmin) {
    alert('Only admin can delete group results.');
    return;
  }

  try {
    await deleteDoc(doc(db, 'groupResults', adminGroup));

    const updatedGroupResults = { ...groupResults };
    delete updatedGroupResults[adminGroup];

    setGroupResults(updatedGroupResults);
    setAdminGroupRanking(groups[adminGroup]);

    await recalculateAllPoints(officialResults, updatedGroupResults);

    alert(`Group ${adminGroup} result deleted and leaderboard updated.`);
  } catch (error) {
    console.error('Delete group result error:', error);
    alert(`Delete group result failed: ${error.message}`);
  }
}
...
<button onClick={deleteOfficialResult}>
  Delete Result
        ...
        <article style={{ gridColumn: '1 / -1', marginTop: 30 }}>
          <ShieldCheck />
          <h3>Admin Panel · Group Results</h3>
          <p>Set the official final group table. This updates group position points and perfect group bonus separately from match results.</p>
            ...
            {groupKeys.map((group) => (
              <button
                ...
                onClick={() => {
                  ...
                  setAdminAwayScore('');
                  setAdminScorers('');
                  setAdminGroupRanking(groupResults[group]?.ranking || groups[group]);
                }}
              >
                <b>Group {group}</b>
                <span>{groupResults[group] ? 'Result saved' : 'No result yet'}</span>
              </button>
            ))}
          </div>

          <div className="ranking-list" style={{ marginTop: 18 }}>
            {adminGroupRanking.map((team, index) => (
              <div className="ranking-row" key={team}>
                <div>
                  <span>{index + 1}</span>
                  <b>{team}</b>
                  <small>Official final position</small>
                </div>

                <div>
                  <button onClick={() => moveAdminGroupTeam(index, -1)}>↑</button>
                  <button onClick={() => moveAdminGroupTeam(index, 1)}>↓</button>
                </div>
              </div>
            ))}
          ...
          <div className="actions">
            <button className="primary" onClick={saveGroupResult}>
              Save Group Result & Update Leaderboard
            </button>

            <button onClick={deleteGroupResult}>
              Delete Group Result
            </button>
          </div>
        </article>
