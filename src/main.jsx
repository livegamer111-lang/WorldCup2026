...
function calculateStandings(group, resultsById) {
  ...
  makeMatches(group, groups[group]).forEach((match, index) => {
...
function groupHasAllResults(group, resultsById) {
  return groups[group].length === 4 && makeMatches(group, groups[group]).every((_, index) => {
    const result = resultsById[matchId(group, index)];
        ...
        {step === 2 && (
          <div>
            ...
            <div className="ranking-list">
              {current.ranking.map((team, index) => (
                <div className="ranking-row" key={team}>
                  ...
                  <div>
                    <button disabled={isGroupTableLocked()} onClick={() => moveTeam(selectedGroup, index, -1)}>↑</button>
                    <button disabled={isGroupTableLocked()} onClick={() => moveTeam(selectedGroup, index, 1)}>↓</button>
                  </div>
                </div>
              ))}
            ...
            <div className="actions">
              ...
              <button className="primary" onClick={() => saveDraftNow(`Group ${selectedGroup} saved`)}>Submit Group</button>
              <button
                onClick={() => {
                  setSelectedGroup(nextGroup);
                  setStep(2);
        ...
        {step === 3 && (
          <div>
            ...
            <div className="actions">
              ...
              <button className="primary" onClick={() => saveDraftNow(`Group ${selectedGroup} matches saved`)}>Submit Matches</button>
              <button
                onClick={() => {
                  setSelectedGroup(nextGroup);
                  setStep(3);
